...
/**
 * Abstract class that declares the basic methods and objects for
 * implementations of <code>LambdaContainerHandler</code>.
 *
 * @param <RequestType>           The expected request object. This is the model
 *                                class that the event JSON is de-serialized to
 * @param <ResponseType>          The expected Lambda function response object.
 *                                Responses from the container will be written
 *                                to this model object
 * @param <ContainerRequestType>  The request type for the wrapped Java
 *                                container
 * @param <ContainerResponseType> The response or response writer type for the
 *                                wrapped Java container
 */
public abstract class LambdaContainerHandler<RequestType, ResponseType, ContainerRequestType, ContainerResponseType>
    ...
    private static JsonMapper objectMapper = JsonMapper.builder().addModule(new AfterburnerModule()).build();
    ...
    protected LambdaContainerHandler(Class<RequestType> requestClass,
        ...
        initializationWrapper = init;
        objectReader = getJsonMapper().readerFor(requestTypeClass);
        objectWriter = getJsonMapper().writerFor(responseTypeClass);
    ...
    protected LambdaContainerHandler(Class<RequestType> requestClass,
            ...
            SecurityContextWriter<RequestType> securityContextWriter,
            ExceptionHandler<ResponseType> exceptionHandler) {
        this(requestClass, responseClass, requestReader, responseWriter, securityContextWriter, exceptionHandler,
    ...
    protected abstract void handleRequest(ContainerRequestType containerRequest,
            ContainerResponseType containerResponse, Context lambdaContext)
            throws Exception;
    ...
    /**
     * Returns the initialization wrapper this container handler will monitor to
     * handle events
     * 
     * @return The initialization wrapper that was passed to the constructor and
     *         this instance will use to decide
     ...
     * 
     * @param wrapper The wrapper this instance will use to decide whether it can
    ...
    /**
     * Configures the library to strip a base path from incoming requests before
     * passing them on to the wrapped
     * framework. This was added in response to issue #34
     * (https://github.com/aws/serverless-java-container/issues/34).
     * When creating a base path mapping for custom domain names in API Gateway we
     * want to be able to strip the base path
    ...
    /**
     * Sets the formatter used to log request data in CloudWatch. By default this is
     * set to use an Apache
     * combined log format based on the servlet request and response object
     * {@link ApacheCombinedServletLogFormatter}.
     * 
    ...
    /**
     * Proxies requests to the underlying container given the incoming Lambda
     * request. This method returns a populated
    ...
    public ResponseType proxy(RequestType request, Context context) {
        ...
        try {
            ...
            if (initializationWrapper != null && initializationWrapper.getInitializationLatch() != null) {
                ...
                if (!initializationWrapper.getInitializationLatch().await(config.getInitializationTimeout(),
                        TimeUnit.MILLISECONDS)) {
                    throw new ContainerInitializationException("Could not initialize framework within the "
                            + config.getInitializationTimeout() + "ms timeout", null);
                }
            }
            ...
            // release all waiting threads. This is safe here because if the count was
            // already 0
     ...
     * <code>JsonMapper</code>
     * to transform the incoming input stream into the given {@link RequestType} and
     * then calls the
     * {@link #proxy(Object, Context)} method to handle the request. The output from
     * the proxy method is
    ...
    public void proxyStream(InputStream input, OutputStream output, Context context)
        ...
        try {
            ...
            getJsonMapper().writeValue(output, exceptionHandler.handle(e));
        } finally {
            output.flush();
            output.close();
        }
